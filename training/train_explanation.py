...
def build_chat_prompt(dp: Dict[str, str], tokenizer) -> Tuple[str, str]:
    """Build the model-specific chat prompt for an explanation sample."""
    msgs = [
...
def tokenize_dp(dp, tokenizer, cutoff_len, train_on_inputs=False):
    """Tokenize a sample and mask prompt tokens from the loss."""
...
def train(
    ...
    print("--- Stage 2: explanation LoRA training (dual-adapter mode) ---")
    ...
    if not train_file or not decision_lora_path:
        raise ValueError(
            "You must provide --decision_lora_path and --train_file")
