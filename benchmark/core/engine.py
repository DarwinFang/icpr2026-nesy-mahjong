class RiichiCalculator:
    ...
    def _cal_shanten_menzu(self, hand, open_melds=0):
        stats = [
    ...
    def calc_shanten(self, hand_or_tiles, open_melds=0):
        if not 0 <= open_melds <= 4:
            raise ValueError("open_melds must be between 0 and 4")
        if not hand_or_tiles:
            hand = self._tiles_to_hand([])
        elif isinstance(hand_or_tiles, list) and isinstance(hand_or_tiles[0], str):
            hand = self._tiles_to_hand(hand_or_tiles)
        else:
            hand = hand_or_tiles
        standard = self._cal_shanten_menzu(hand, open_melds)
        if open_melds:
            return standard
        return min(standard,
    ...
    def calc_ukeire(self, hand_or_tiles, open_melds=0):
        if not hand_or_tiles:
            hand = self._tiles_to_hand([])
        elif isinstance(hand_or_tiles, list) and isinstance(hand_or_tiles[0], str):
            hand = self._tiles_to_hand(hand_or_tiles)
        ...
        if tile_count % 3 == 1:
            return self._ukeire_draw(hand, open_melds)
        else:
            return self._ukeire_discard(hand, open_melds)

    def _ukeire_draw(self, hand, open_melds=0):
        current_shanten = self.calc_shanten(hand, open_melds)
        valid_tiles = {}
        total_count = 0
        ...
        for i in range(4):
            for j in range(len(hand[i])):
                ...
                hand[i][j] += 1
                new_shanten = self.calc_shanten(hand, open_melds)
                hand[i][j] -= 1
    ...
    def _ukeire_discard(self, hand, open_melds=0):
        current_shanten = self.calc_shanten(hand, open_melds)
        normal_discard = {}
        receding_discard = {}
        ...
        for i in range(4):
            for j in range(len(hand[i])):
                if hand[i][j] > 0:
                    tile_name = self.TILE_NAMES[i][j]
                    hand[i][j] -= 1
                    result = self._ukeire_draw(hand, open_melds)
                    hand[i][j] += 1
